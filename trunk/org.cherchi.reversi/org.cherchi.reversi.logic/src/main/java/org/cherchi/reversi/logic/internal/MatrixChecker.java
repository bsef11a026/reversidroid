...
public class MatrixChecker {
	...
	public boolean isEnclosingRight(int player, int col, int row) {
		...
		if (col < GameLogicImpl.COLS - 1) {
			...
			// opponent
			if (this.gameMatrix[col + 1][row] == opponent(player)) {

				int value = this.getFirstPositionDifferentThan(
						opponent(player), col + 1, row, Direction.X.RIGHT,
						Direction.Y.NONE);
				// only encloses if the first different than opponent is an own
				// chip
				enclosing = value == player;
	...
	public boolean isEnclosingLeft(int player, int col, int row) {
		...
		if (col > 1) {
			...
			if (this.gameMatrix[col - 1][row] == opponent(player)) {
				...
				int value = this.getFirstPositionDifferentThan(
						opponent(player), col - 1, row, Direction.X.LEFT,
						Direction.Y.NONE);
				// only encloses if the first different than opponent is an own
				// chip
				enclosing = value == player;
	...
	public boolean isEnclosingLeftAndUp(int player, int col, int row) {
		...
		if (col > 1 && row > 1) {
			...
			// impossible enclose if adjacent chip is not
			// opponent
			if (this.gameMatrix[col - 1][row - 1] == opponent(player)) {

				// searching left and up for an own chip
				int value = this.getFirstPositionDifferentThan(
						opponent(player), col - 1, row - 1, Direction.X.LEFT,
						Direction.Y.UP);
				// only encloses if the first different than opponent is an own chip
				enclosing = value == player;
			}
		}
		return enclosing;
	...
	public boolean isEnclosingRightAndDown(int player, int col, int row) {
		...
		if (col < GameLogic.COLS - 1 && row < GameLogic.ROWS - 1) {
			...
			if (this.gameMatrix[col + 1][row + 1] == opponent(player)) {
				...
				// searching left and up for an own chip
				int value = this.getFirstPositionDifferentThan(
						opponent(player), col + 1, row + 1, Direction.X.RIGHT,
						Direction.Y.DOWN);
				// only encloses if the first different than opponent is an own chip
				enclosing = value == player;
			}
	...
	private boolean isEmpty(int col, int row) {
		...
		return this.gameMatrix[col][row] == 0;
	}
}
